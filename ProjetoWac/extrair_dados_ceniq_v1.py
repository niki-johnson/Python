...
def buscar_oleos_graxas(conteudo):
    padrao = r'(Óleos e graxas|Substâncias solúveis em hexano)\s+(\d+.\d+|<\s*0,\d+)'
    #+([\d\.]+|[\d\,]+|[<\d]+|[\d,\d X \d]+|[<\d?,\d+>]+)
    oleo_graxa = re.findall(padrao, conteudo['content'])
...
def buscar_dbo(conteudo):
    padrao = r'(DBO)\s+(\d+.\d+)'
    #+([\d\.]+|[\d\,]+|[<\d]+|[\d,\d X \d]+|[<\d?,\d+>]+)
    dbo = re.findall(padrao, conteudo['content'])
...
def buscar_dqo(conteudo):
    padrao = r'(DQO)\s+(\d+.\d+)'
    #+([\d\.]+|[\d\,]+|[<\d]+|[\d,\d X \d]+|[<\d?,\d+>]+)
    dqo = re.findall(padrao, conteudo['content'])
...
def buscar_pH(conteudo):
    padrao = r'(pH)\s+(\d+.\d+)'
    #+([\d\.]+|[\d\,]+|[<\d]+|[\d,\d X \d]+|[<\d?,\d+>]+)
    pH = re.findall(padrao, conteudo['content'])
...
def buscar_cloro(conteudo):
    padrao = r'(Cloro [Rr]esidual [Ll]ivre)\s*+(<\s*0,\d+|\d+.\d+)'
    #+([\d\.]+|[\d\,]+|[<\d]+|[\d,\d X \d]+|[<\d?,\d+>]+)
    cloro = re.findall(padrao, conteudo['content'])
...
def buscar_od(conteudo):
    ...
    #Oxigênio dissolvido|
    resultados = re.findall(padrao, conteudo['content'])
...
def buscar_solidos_sed(conteudo):
    ...
    #+([\d\.]+|[\d\,]+|[<\d]+|[\d,\d X \d]+|[<\d?,\d+>]+)
    solidos = re.findall(padrao, conteudo['content'])
...
def buscar_coliformes(conteudo):
    ...
    coliformes = re.findall(padrao, conteudo['content'])
